...
/*
URL PATH : /panels/question_cat/
LOCATION : /application/controllers/panels/question_cat.php
*/
...
{  
	...
	{ 
		...
		$d = new stdClass();  
		$d->where = array('cat_id' => $v1,'web_id' => $_SESSION['panel_id']);
		...
		{  
			$data = '';
			$count_it = true;
			$header = ''.$this->lang->line('no.').','.$this->lang->line('name').','.$this->lang->line('email').','.$this->lang->line('telephone').','.$this->lang->line('ip_address').','; 
			...
			$header = $header . ''.$this->lang->line('last_create').'';
		...
		else
		{
			?><meta charset="utf-8" />
            <script>
			alert('<?php echo $this->lang->line('no_data') ?>');
			window.history.back();
			</script>
            <?php
			exit;	
