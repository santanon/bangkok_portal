
<div class="group-content">
    <div class="group-content-left">
        <div class="group-link-mainbkk">
            <a href="">
                <div class="icon-bkk">
                    <img src="http://127.0.0.1:8000/template1/assets/images/icons/icon-bkk.png" alt="icon">
                </div>
                <div class="text-bkk">
                    <label>กรุงเทพมหานคร</label>
                    Bangkok.go.th
                </div>
            </a>
        </div>
        <a href="#" class="btn btn-outline-primary">เข้าสู่เว็บไซต์</a>
    </div>
    
    <div class="group-content-right">
        <div class="online-wrapper">
            <div class="group-online">
                <div class="text-online">Online</div>
            </div>
            <div class="num-online">1.6K</div>
        </div>
    </div>
</div>

<div class="group-footer bg-color-primary">
    <div class="title-footer">
        <h2 class="title">
            สำนักงานเลขานุการสภากรุงเทพมหานคร 
        </h2>
        <!-- <p class="desc">
            The Bangkok Metropolitan Council Secretariat
        </p> -->
    </div>
    <div class="group-content-footer">
        <!-- <div class="icon-content">
            <img src="{{ asset('template1/assets/images/icons/icon-map.png')}}" alt="icon">
        </div> -->
        <div class="text-content">
            173 ศาลาว่าการกรุงเทพมหานคร (เสาชิงช้า) ถนนดินสอ แขวงเสาชิงช้า เขตพระนคร กรุงเทพมหานคร 10200
        </div>
    </div>
    <div class="footer-bottom">
        <div class="sitemap">
            <ul>
                <li>
                    <a href="/sitemap">แผนผังเว็บไซต์</a>
                </li>
                <li>
                    <a href="">ข้อตกลงการใช้ระบบ</a>
                </li>
            </ul>
        </div>
    </div>
</div>

<div class="group-footer-contact">
    <div class="copyright">
        สงวนลิขสิทธิ์ พ.ศ.2563
    </div>  
    <div class="group-content-footer">
        <div class="icon-content">
            <img src="{{ asset('template1/assets/images/icons/icon-tel.svg')}}" alt="icon">
        </div>
        <div class="text-content">
            โทรศัพท์กลาง
            <a class="link-tel" href="tel:022212141">0-2221-2141-69</a>
        </div>
    </div> 
    <div class="group-content-footer">
        <div class="icon-content">
            <img src="{{ asset('template1/assets/images/icons/icon-fax.svg')}}" alt="icon">
        </div>
        <div class="text-content">
            โทรสาร
            <a class="link-tel" href="javascript: void(0)">0-2224-3038</a>
        </div>
    </div>   
    <div class="social-wrapper d-flex align-items-center">
        <ul>
            <li class="facebook">
                <a href="" title="facebook">
                    <img src="{{ asset('template1/assets/images/icons/icon-social-facebook.svg')}}" alt="icon">
                </a>
            </li>
            <li class="twitter">
                <a href="" title="twitter">
                    <img src="{{ asset('template1/assets/images/icons/icon-social-twitter.svg')}}" alt="icon">
                </a>
            </li>
            <li class="line">
                <a href="" title="line">
                    <img src="{{ asset('template1/assets/images/icons/icon-social-line.svg')}}" alt="icon">
                </a>
            </li>
        </ul>
    </div>     
</div>


<div class="gototop">
    <div class="icon-gototop">
        <img src="{{ asset('template1/assets/images/icons/icon-gototop.png')}}" alt="icon">
    </div>
</div>

<div class="bg-canvas"></div>
