...
class elFinder {
	...
	const ERROR_UPLOAD_TRANSFER   = 'errUploadTransfer1';   // '"$1" transfer error.'
	const ERROR_UPLOAD_TRANSFER2   = 'errUploadTransfer2';   // '"$1" transfer error.'
	const ERROR_UPLOAD_TRANSFER3   = 'errUploadTransfer3';   // '"$1" transfer error.'
	const ERROR_UPLOAD_TRANSFER4   = 'errUploadTransfer4';   // '"$1" transfer error.'
...
//		}
		...
		foreach ($files['name'] as $i => $name) {
			if (($error = $files['error'][$i]) > 0) {				
				$result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $name, $error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE ? self::ERROR_UPLOAD_FILE_SIZE : self::ERROR_UPLOAD_TRANSFER2);
			...
			if ($name === 'blob') {
				if ($chunk) {
					if ($tempDir = $this->getTempDir($volume->getTempPath())) {
						...
						$result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $chunk, self::ERROR_UPLOAD_TRANSFER3);
			...
			//$tmpname = "C:/Windows/Temp/".substr($tmpname,13);
			$tmpname = "C:/session/".substr($tmpname,13);
			
			if (($fp = fopen($tmpname, 'rb')) == false) {
				$result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $name, self::ERROR_UPLOAD_TRANSFER4);
				$this->uploadDebug = 'Upload error: unable open tmp file';
